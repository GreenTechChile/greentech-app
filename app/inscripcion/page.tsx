  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const parrafosDeclaracion = [
        `Yo, ${form.nombre}, cédula nacional de identidad ${rut}, fecha de nacimiento ${form.fecha_nacimiento.split('-').reverse().join('-')}, estado civil ${form.estado_civil}, de profesión u oficio ${form.profesion}, con domicilio en ${domicilio}, comuna de ${form.comuna}, ciudad de ${form.ciudad}, correo electrónico ${form.email}, teléfono móvil ${form.telefono},`,
        `diagnosticado/a con ${form.diagnostico}, por este acto y por el presente instrumento, VENGO EN DECLARAR QUE:`,
        `PRIMERO: Debido a mi patología, y con el afán de mejorar mi calidad de vida, declaro ser usuario Medicinal de Cannabis. Además, señalo que cuento con receta médica, determinada con el número ${form.folio_receta}, que justifica mi uso de tipo medicinal hasta la cantidad de ${form.cuota_mensual} gramos mensuales, según lo prescrito en la receta médica que justifica el tratamiento. La presente receta se encuentra vigente hasta la fecha de ${form.vencimiento_receta.split('-').reverse().join('-')}, la cual ha sido extendida por el(la) doctor(a) ${form.medico_nombre}, documento de identidad número ${form.medico_rut}.`,
            ...
            return (
              <div>
                ...
                <div style={{border:'1px solid #e5e7eb',borderRadius:10,height:340,overflowY:'auto',padding:16,fontSize:11.5,lineHeight:1.8,color:'#374151',marginBottom:14,background:'#fafafa'}}
                  ...
                  <p style={{marginTop:10}}><strong>PRIMERO:</strong> Debido a mi patología, y con el afán de mejorar mi calidad de vida, <strong>declaro ser usuario Medicinal de Cannabis</strong>. Además, señalo que cuento con receta médica, determinada con el número <strong>{form.folio_receta||'_______________'}</strong>, que justifica mi uso de tipo medicinal hasta la cantidad de <strong>{form.cuota_mensual||'___'} gramos mensuales</strong>, según lo prescrito en la receta médica que justifica el tratamiento. La presente receta se encuentra vigente hasta la fecha de <strong>{form.vencimiento_receta?form.vencimiento_receta.split('-').reverse().join('-'):'_______________'}</strong>, la cual ha sido extendida por el(la) doctor(a) <strong>{form.medico_nombre||'_______________'}</strong>, documento de identidad número <strong>{form.medico_rut||'_______________'}</strong>.</p>
